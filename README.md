![docker image](https://github.com/GarzonDiegoFEUP/nomad-oasis-inl/actions/workflows/docker-publish.yml/badge.svg)
...
2. Clone the repository or download the repository as a zip file.
    ...
    ```sh
    git clone https://github.com/GarzonDiegoFEUP/nomad-oasis-inl.git
    cd nomad-oasis-inl
    ```
    ...
    ```sh
    curl-L -o nomad-oasis-inl.zip "https://github.com/GarzonDiegoFEUP/nomad-oasis-inl/archive/main.zip"
    unzip nomad-oasis-inl.zip
    cd nomad-oasis-inl
    ```
...
your `docker-compose.yaml` with `ghcr.io/garzondiegofeup/nomad-oasis-inl:main` for the services
...
By default, no plugins are included in this distribution. You can find a list of available NOMAD plugins [here](https://nomad-lab.eu/prod/v1/oasis/gui/search/plugins). For a list of official plugins provided by FAIRmat, please see [here](https://github.com/GarzonDiegoFEUP/.github/blob/main/profile/README.md). For inspiration, you can also check the list of [plugins that are installed on the production NOMAD deployment hosted by FAIRmat](https://gitlab.mpcdf.mpg.de/nomad-lab/nomad-distro/-/raw/main/pyproject.toml?ref_type=heads).
...
```sh
docker pull ghcr.io/garzondiegofeup/nomad-oasis-inl/jupyter:main
```
...
By default, documentation is built using the [nomad-docs](https://github.com/GarzonDiegoFEUP/nomad-docs) repository. However, if you'd like to customize the documentation for your Oasis instance, you can easily do so.

1. First, [fork the nomad-docs repository](https://github.com/GarzonDiegoFEUP/nomad-docs/fork).
...
_I get an_ `Error response from daemon: Head "https://ghcr.io/v2/GarzonDiegoFEUP/nomad-oasis-inl/manifests/main": unauthorized`
