...
c = get_config()  # type: ignore  # pylint: disable=undefined-variable  # noqa: F821
...
def pre_spawn(spawner):
    ...
    # Allow the operator to override NOMAD_CLIENT_URL with an internal Docker
    # service URL (e.g. http://app:8000/nomad-oasis/api/v1) so that spawned
    # Jupyter containers can reach the API without routing through the external
    # hostname, which may not be resolvable or reachable inside the Docker network.
    import os as _os  # noqa: PLC0415
    _internal_client_url = _os.environ.get('NOMAD_NORTH_INTERNAL_CLIENT_URL')
    if _internal_client_url and 'NOMAD_CLIENT_URL' in spawner.environment:
        spawner.log.info(
            'Overriding NOMAD_CLIENT_URL with internal URL: %s',
            _internal_client_url,
        )
        spawner.environment['NOMAD_CLIENT_URL'] = _internal_client_url
    ...
    if tool:
        ...
        if tool.get('seccomp_unconfined'):
            extra_host_config['security_opt'] = ['seccomp=unconfined']
